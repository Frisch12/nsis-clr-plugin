...
public:
	static Dictionary<String^, Assembly^>^ loadedAssemblies = nullptr;
};

System::Reflection::Assembly ^ LoadCachedAssembly(String ^filename)
{
	if(GlobalObjects::loadedAssemblies == nullptr)
	{
		GlobalObjects::loadedAssemblies = gcnew Dictionary<String^, Assembly^>();
	}
	if (GlobalObjects::loadedAssemblies->ContainsKey(filename))
		return GlobalObjects::loadedAssemblies[filename];
	Assembly ^assembly = LoadAssembly(filename);
	GlobalObjects::loadedAssemblies->Add(filename, assembly);
	return assembly;
}
...
char* CallCLR(string DllName, string ClassWithNamespace, string MethodName, vector<string> Args)
{
	try
	{
		...
		System::Reflection::Assembly ^assembly = LoadCachedAssembly(".\\" + sDLLName);
		if (assembly == nullptr)
...
extern "C" __declspec(dllexport) void Destroy(HWND hwndParent, int string_size, 
									  ...
									  extra_parameters *extra)
{
	GlobalObjects::loadedAssemblies->Clear();
	GlobalObjects::loadedAssemblies = nullptr;
}
